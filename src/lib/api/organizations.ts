 ...
 */
export async function hasSection(organizationId: string, section: 'planes' | 'helicopters' | 'residences' | 'watercraft'): Promise<boolean> {
 ...
 */
export async function getActiveSections(organizationId: string): Promise<('planes' | 'helicopters' | 'residences' | 'watercraft')[]> {
  const subscription = await getSubscription(organizationId);
